...
function LilacResult(props){
    ...
    const userOneChecker = async () => {
        const checkUserOne = await db.collection(`${진행중회차}lilac`).where("userOne.Nick", "==", 닉네임).get()
        try {
    ...
    const userTwoChecker = async () => {
        const checkUserTwo = await db.collection(`${진행중회차}lilac`).where("userTwo.Nick", "==", 닉네임).get()
    ...
    const 지난회차참여자userOneChecker = async () => {
        const checkUserOne = await db.collection(`${지난회차}lilac`).where("userOne.Nick", "==", 닉네임).get()
        try {
            checkUserOne.forEach((doc) => {
                문서번호변경(doc.id)
                유저1변경(doc.data()['userOne'])
    ...
    const 지난회차참여자userTwoChecker = async () => {
        const checkUserOne = await db.collection(`${지난회차}lilac`).where("userTwo.Nick", "==", 닉네임).get()
        try {
            checkUserOne.forEach((doc) => {
                문서번호변경(doc.id)
                유저1변경(doc.data()['userOne'])
